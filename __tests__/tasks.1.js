...
describe('Generate colors', () => {
    it("New color to guess is generated when 'New Colors' is clicked", async () => {
        const currentColor = await page.$eval('#color-display', el => el.textContent);
...
describe('Easy mode', () => {
    it("When 'Easy' mode is selected, only 3 color options are displayed", async () => {
        ...
        const visibleSquares = await page.evaluate(() => {
            return Array.from(document.querySelectorAll('.square')).filter(square => square.style.display !== 'none').length;
        })
...
describe('Hard mode', () => {
    it("When 'Hard' mode is selected, 6 color options are displayed", async () => {
        ...
        const visibleSquares = await page.evaluate(() => {
            return Array.from(document.querySelectorAll('.square')).filter(square => square).length;
        })
...
describe('Guess meggage', () => {
    it("A 'Correct' message is displayed in the `#message` container when user guesses color correctly", async () => {
        const colorToGuess = await page.$eval('#color-display', el => el.textContent);
        const squares = await page.$$('.square');
        const squareColors = await Promise.all(squares.map(square => square.evaluate(el => el.style.backgroundColor)));
